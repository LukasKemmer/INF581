...
\begin{figure}[h]
	...
	\caption{\label{model}Example of a supply chain network with $k=5$ warehouses ($s_1, .., s_5$) and the factory ($s_0$).}
\end{figure}
...
\begin{equation}
	\begin{split}
		d_{i,t} = &\left\lceil \frac{d_{max}}{2} \sin\left( \frac{2 \pi (t + 2i) }{12}\right) + \frac{d_{max}}{2} + \epsilon \right\rceil
	\end{split}
\end{equation}
with $\left\lceil \cdot \right\rceil$ the ceiling function and $P(\epsilon=0) = P(\epsilon=1) = 0.5$. We will include $d_{last}, d_{hist} \in D$ in our state space where for some time $t$ we have the tuple $(s, d_{last}, d_{hist})$ with $s = s_t$, $d_{last} = d_{t-1}$ and $d_{hist} = d_{t-2}$. Note that this implies that the demand $d_t$ of period $t$ is not observed by the agent. This means, that the demand of a period will be realized after observing the stock levels $s_t$ and can only be observed in the next period $t+1$.
...
\begin{defn} \label{def:modell} \
	\begin{itemize}
		\item[1.] $S \times D^3 \coloneqq \prod\limits_{j=0}^{k} \{s_j \in \mathbb{Z} \ | \ s_j \leq c_j\} \times D^3, c_j \in \mathbb{N}$ the state space consisting of elements $((s, d_{last}, d_{hist}), d)$;
		\item[2.] $ A \coloneqq  \{0, ..., \rho_{max} \} \times \mathbb{N}_0^k$ the action space consisting of elements $a = (a_0, ..., a_k)$;
		\item[3.] $ \mathcal{X} (s) \coloneqq \{0, ..., \rho_{max} \} \times \{a \in \mathbb{N}_0^k \ | \ \sum_{j=1}^{k}a_j \leq s_0  \}$, the set of all feasible actions in a state $s$ and $\mathcal{X} \coloneqq \{(s, d_{last}, d_{hist}, d , a) \in S \times D^3 \times A \ | \ a \in \mathcal{X} (s)\}$;
...
%				\end{split}
%			\end{equation*}
		\item[5.] $ Q: \mathcal{X} \times S \times D^3 \rightarrow [0,1]  $ the transition probabilities with $Q(s', d'\ |\ s, d, a) \coloneqq q_d(d')$ for $s' = T(s, d_{last}, d_{hist}, d, a)$ and $0$ otherwise;
