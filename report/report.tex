...
\end{figure}
\newline
Furthermore we use several different stepsize formulars from \cite{PowellADP} that are used to update some of the agents parameters such as the learning rate $\alpha$. 
The idea of specific stepsize rules is to allow the agent to go through a longer learning phase in the beginning with higher learning rates and then converge in the long term to stepsizes close to $0$. %This behavior can be seen in \ref{fig:stc_stepsize}. that shows the learning rates generated from 
...
\textbf{The REINFORCE algorithm} is is based on a parametrized policy that has to be optimized. The optimization of finding the maximum expected reward is achieved with a gradient ascent technique. We assume our policy to be parametrized with a softmax function for multiple actions. The softmax function works as a stochastic policy that can under convergence turn into a deterministic policy. Our policy is thus defined in the following way:
\begin{equation}
    \label{simple_equation}
    p( a = a^{(k)}| s) = \pi_\Theta(a = a^{(k)},s) = \frac{e^{\Phi(s)^T w_{a^{(k)}}}\cdot f(a^{(k)}|s)}{\sum_{j=1}^{n_a} e^{\Phi(s)^T w_{a^{(j)}}}\cdot f(a^{(j)}|s)} 
\end{equation}
where 
\begin{equation}
   f(a^{(j)}|s) =
   ...
   \end{cases}
\end{equation}
$n_a$ is the number of possible actions $n_a = n_a_{per_store}^{(k+1)}$ ($n_a=3$  number of actions per factory/warehouse, $k=3$ number of stores). The parameter matrix $\Theta$ is assembled the following way:
\begin{equation}
